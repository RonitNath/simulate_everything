...
Operations is a priority-weighted task allocator, not a collection of named
subsystems. All tasks (farming, building, crafting, soldiering, trading,
settling, patrolling) are on equal footing.
...
Output: task assignments and resource allocation commands.
...
what's feasible. Shortfalls trigger ProduceEquipment commands.

### Tactical Layer — Matchup Reasoning

Runs every tick for stacks within 300m engagement detection radius of an
enemy entity. Detection is a spatial query per stack (dozens of queries,
not thousands).
