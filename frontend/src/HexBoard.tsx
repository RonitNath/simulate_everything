...
const HexBoard: Component<HexBoardProps> = (props) => {
  ...
  // Build unit lookup: "row,col" -> strongest unit
  const unitMap = createMemo(() => {
    const map = new Map<string, V2UnitSnapshot>();
    // Convert axial (q,r) to offset (row,col) matching engine's axial_to_offset
    for (const u of props.units) {
      const row = u.r;
      const col = u.q + (u.r - (u.r & 1)) / 2;
      const key = `${row},${col}`;
      const existing = map.get(key);
      if (!existing || u.strength > existing.strength) {
        map.set(key, u);
      }
    }
    return map;
            ...
            {(col) => {
              ...
              return (
                <g>
                  ...
                  {/* Strength label */}
                  {props.showStrength && !unit.is_general && s > 10 && (
                    <text
                      x={cx}
                      y={cy + s * 0.1}
                      text-anchor="middle"
