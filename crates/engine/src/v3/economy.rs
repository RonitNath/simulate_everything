...
use super::state::{
    EntityBuilder, GameState, Resource, ResourceType, Role, Structure, StructureType,
    TaskAssignment,
};
use super::weapon;
...
pub fn produce_resources(state: &mut GameState, dt: f32) {
    ...
    for entity in state.entities.values() {
        ...
        match task {
            TaskAssignment::Farm { site } => {
                if valid_structure(state, *site, owner, StructureType::Farm) {
                    food_deltas[owner as usize] += FARM_OUTPUT_PER_TASK * dt;
                }
            }
            TaskAssignment::Workshop { site } => {
                if valid_structure(state, *site, owner, StructureType::Workshop) {
    ...
    for owner in 0..state.num_players {
        ...
        if food > 0.0 {
            add_player_stockpile(state, owner, ResourceType::Food, food);
        }
        let material = material_deltas[owner as usize];
        if material > 0.0 {
            add_player_stockpile(state, owner, ResourceType::Material, material);
        }
    }
}
...
pub fn consume_food(state: &mut GameState, dt: f32) {
    ...
    for owner in 0..state.num_players {
        ...
        let available = player_stockpile_amount(state, owner, ResourceType::Food);
        let consumed = available.min(need);
        if consumed > 0.0 {
            consume_player_stockpile(state, owner, ResourceType::Food, consumed);
        }
...
pub fn produce_equipment_now(
    ...
    if !valid_structure(state, workshop, owner, StructureType::Workshop) {
        return false;
    }

    let cost = item_cost(item_type);
    if player_stockpile_amount(state, owner, ResourceType::Material) < cost {
        return false;
    }
    ...
    consume_player_stockpile(state, owner, ResourceType::Material, cost);
...
pub fn bootstrap_shared_economy_layout(state: &mut GameState) {
    ...
    for (owner, center) in villages {
        ...
        let _ = ensure_stockpile_resource(state, owner, ResourceType::Food);
        let _ = ensure_stockpile_resource(state, owner, ResourceType::Material);
    }
}
...
pub fn player_stockpile_amount(state: &GameState, owner: u8, resource_type: ResourceType) -> f32 {
    state
        .entities
        .values()
        .filter(|entity| entity.owner == Some(owner))
        .filter_map(|entity| entity.resource.as_ref())
        .filter(|resource| {
            matches!(
...
fn consume_player_stockpile(
    ...
    let mut resources: Vec<(EntityKey, u32)> = state
        ...
        .filter_map(|(key, entity)| {
            let resource = entity.resource.as_ref()?;
            let matches = matches!(
                (resource_type, resource.resource_type),
                (ResourceType::Food, ResourceType::Food)
                    | (ResourceType::Material, ResourceType::Material)
            );
            matches.then_some((key, entity.id))
        })
        .collect();
    ...
    if let Some(existing) = state.entities.get(container).and_then(|entity| {
        entity.contains.iter().copied().find(|child| {
            state
                .entities
                .get(*child)
                .and_then(|entity| entity.resource.as_ref())
                .map(|resource| resource.resource_type == resource_type)
                .unwrap_or(false)
        })
    }) {
...
fn canonical_stockpile_container(state: &GameState, owner: u8) -> Option<EntityKey> {
    fn structure_rank(structure_type: StructureType) -> u8 {
        match structure_type {
            StructureType::Village | StructureType::City | StructureType::Depot => 0,
            _ => 1,
        }
    }
    ...
    state
        ...
        .filter_map(|(key, entity)| {
            let structure = entity.structure.as_ref()?;
            Some((key, structure_rank(structure.structure_type), entity.id))
        })
        .min_by_key(|(_, rank, id)| (*rank, *id))
        .map(|(key, _, _)| key)
}

fn valid_structure(state: &GameState, site: EntityKey, owner: u8, expected: StructureType) -> bool {
    state
        .entities
        .get(site)
        .map(|entity| {
            entity.owner == Some(owner)
                && entity
                    ...
                    .as_ref()
                    .map(|structure| {
                        structure.structure_type == expected && structure.build_progress >= 1.0
                    })
                    .unwrap_or(false)
        })
        .unwrap_or(false)
...
mod tests {
    ...
    use super::super::state::{Combatant, GameState, Person};
    use super::*;
    ...
    fn stockpile_helpers_create_and_reuse_resources() {
        ...
        let village = spawn_structure(&mut state, 0, StructureType::Village, Vec3::ZERO);
        let food = ensure_stockpile_resource(&mut state, 0, ResourceType::Food).unwrap();
        let same_food = ensure_stockpile_resource(&mut state, 0, ResourceType::Food).unwrap();
    ...
    fn farmer_task_produces_food() {
        ...
        assert!(player_stockpile_amount(&state, 0, ResourceType::Food) >= 2.0);
    }

    #[test]
    fn workshop_task_produces_material() {
        let mut state = test_state();
        let _village = spawn_structure(&mut state, 0, StructureType::Village, Vec3::ZERO);
        let workshop = spawn_structure(
            &mut state,
            0,
            StructureType::Workshop,
            Vec3::new(10.0, 0.0, 0.0),
        ...
        assert!(player_stockpile_amount(&state, 0, ResourceType::Material) >= 1.0);
    }
    ...
    fn food_consumption_drains_stockpile() {
        ...
        assert!(player_stockpile_amount(&state, 0, ResourceType::Food) < 10.0);
    }
    ...
    fn food_shortage_drains_stamina_not_blood() {
        let mut state = test_state();
        let village = spawn_structure(&mut state, 0, StructureType::Village, Vec3::ZERO);
        let _food = ensure_stockpile_resource(&mut state, 0, ResourceType::Food).unwrap();
        ...
        let farms = state
            ...
            .filter(|entity| {
                entity
                    .structure
                    .as_ref()
                    .map(|structure| structure.structure_type == StructureType::Farm)
                    .unwrap_or(false)
            })
            .count();
        let workshops = state
            ...
            .filter(|entity| {
                entity
                    .structure
                    .as_ref()
                    .map(|structure| structure.structure_type == StructureType::Workshop)
                    .unwrap_or(false)
            })
            .count();
        assert_eq!(farms, 2);
