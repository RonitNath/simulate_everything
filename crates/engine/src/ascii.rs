use std::fmt;
use crate::state::{GameState, Cell, Tile};
...
fn player_char(player: u8, general: bool) -> char {
    let base = b'a' + player;
    if general { (base - 32) as char } else { base as char }
}
    ...
    match (cell.tile, cell.owner) {
        ...
        (Tile::Empty | Tile::City | Tile::General, None) => {
            if cell.tile == Tile::City {
                ...
                buf.push('c');
                for _ in 0..(col_w - 1 - army_str.len()) { buf.push(' '); }
        ...
        (tile, Some(p)) => {
            ...
            if col_w > 1 + army_str.len() {
                buf.push(marker);
                for _ in 0..(col_w - 2 - army_str.len()) { buf.push(' '); }
            }
            buf.push_str(&army_str);
        }
    }
...
fn col_width(grid: &[Cell]) -> usize {
    ...
    let digits = if max_army == 0 { 1 } else { (max_army as f64).log10().floor() as usize + 1 };
...
pub fn screenshot(
    ...
    if let Some(stats) = stats {
        for s in stats {
            ...
            out.push_str(&format!(" | {}: {} land {} army{}", ch, s.land, s.armies, alive));
        }
    }
    out.push('\n');

    // Column headers
    for _ in 0..row_label_w { out.push(' '); }
    for c in 0..width {
        let label = format!("{}", c);
    ...
    for r in 0..height {
        let label = format!("{}", r);
        for _ in 0..(row_label_w - label.len()) { out.push(' '); }
        out.push_str(&label);
        for c in 0..width {
            if c > 0 { out.push(' '); }
...
impl Frame {
    pub fn ascii(&self, width: usize, height: usize) -> FrameView<'_> {
        FrameView { frame: self, width, height }
    }
}
...
mod tests {
    ...
    fn basic_screenshot() {
        let grid = vec![
            Cell { tile: Tile::General, owner: Some(0), armies: 1 },
            Cell::empty(),
            Cell::mountain(),
            Cell::empty(),
            Cell::empty(),
            Cell { tile: Tile::City, owner: None, armies: 40 },
            Cell::mountain(),
            Cell::empty(),
            Cell { tile: Tile::General, owner: Some(1), armies: 12 },
        ];
    ...
    fn owned_city_marker() {
        let grid = vec![
            Cell { tile: Tile::City, owner: Some(0), armies: 7 },
        ];
        let out = screenshot(1, 1, &grid, 5, None);
        assert!(out.contains("a~"), "owned city should have ~ marker: {}", out);
    }
    ...
    fn dead_player_stats() {
        ...
        let stats = vec![
            PlayerStats { player: 0, land: 10, armies: 50, alive: true },
            PlayerStats { player: 1, land: 0, armies: 0, alive: false },
        ];
